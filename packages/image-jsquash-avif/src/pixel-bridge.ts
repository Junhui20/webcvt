 ...
 * is needed to convert between canvas-native formats and AVIF. The AVIF-typed
...
const errorHooks: PixelBridgeErrorHooks = {
  encodeContextError: () =>
    new AvifEncodeError('Could not get 2D context from canvas for pixel bridge (imageDataToBlob).'),
  decodeContextError: () =>
    new AvifDecodeError('Could not get 2D context from canvas for pixel bridge (blobToImageData).'),
  toBlobNullError: () =>
    new AvifEncodeError(
  ...
  dimensionsTooLargeError: (width, height, maxPixels) =>
    new AvifDimensionsTooLargeError(width, height, maxPixels),
};
