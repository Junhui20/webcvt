/**
 * Tests for pixel-bridge.ts — environment detection plus the decode-only
 * blobToImageData path exercised with mocked OffscreenCanvas / createImageBitmap.
 */

import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import { OxipngDimensionsTooLargeError } from './errors.ts';
...
describe('blobToImageData', () => {
  ...
  it('decodes a blob to ImageData and closes the bitmap', async () => {
    ...
    expect(await blobToImageData(new Blob(['x'], { type: 'image/jpeg' }))).toBe(fakeImageData);
    expect(close).toHaveBeenCalledOnce();
  });

  it('uses the HTMLCanvasElement fallback when OffscreenCanvas is absent', async () => {
    ...
    setGlobal('document', {
      createElement: vi.fn(() => ({
        width: 0,
        height: 0,
        getContext: () => ({ drawImage: vi.fn(), getImageData: () => fakeImageData }),
      })),
    });
    setGlobal(
      'createImageBitmap',
    ...
    expect(await blobToImageData(new Blob(['x']))).toBe(fakeImageData);
    expect(close).toHaveBeenCalledOnce();
  });

  it('throws OxipngDimensionsTooLargeError and still closes the bitmap when too large', async () => {
    const close = vi.fn();
    setGlobal(
      'createImageBitmap',
