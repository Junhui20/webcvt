...
import { MozjpegDimensionsTooLargeError } from './errors.ts';
...
describe('imageDataToBlob', () => {
  ...
  it('falls back to HTMLCanvasElement.toBlob when OffscreenCanvas is absent', async () => {
    ...
    expect(await imageDataToBlob(imageData, 'image/png', 0.8)).toBe(fakeBlob);
  });
  ...
  it('rejects when HTMLCanvasElement.toBlob yields null', async () => {
    ...
    await expect(imageDataToBlob(imageData, 'image/png')).rejects.toThrow(/null/);
  });
...
describe('blobToImageData', () => {
  ...
  it('throws MozjpegDimensionsTooLargeError and still closes the bitmap when too large', async () => {
    const close = vi.fn();
    setGlobal(
      'createImageBitmap',
