 ...
 * Do NOT import from container-webm. This copy evolves independently until
 * Phase 3 extracts @webcvt/ebml.
 *
...
function vintWidth(firstByte: number, offset: number): 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 {
  if (firstByte === 0) {
    throw new MkvVintError(offset, 'first byte is 0x00 (invalid VINT — exceeds 8-byte max width)');
  }
 ...
 *
 * @throws MkvVintError if the encoding is invalid.
 */
export function readVintId(bytes: Uint8Array, offset: number): EbmlVint {
  if (offset >= bytes.length) {
    throw new MkvVintError(offset, 'offset past end of buffer');
  }
  const first = bytes[offset] as number;
  const width = vintWidth(first, offset);

  if (offset + width > bytes.length) {
    throw new MkvVintError(offset, `VINT width ${width} exceeds buffer length`);
  }
  ...
  if (width > 4) {
    throw new MkvVintError(offset, `ID VINT width ${width} exceeds maximum ID width of 4`);
  }
...
export function readVintSize(bytes: Uint8Array, offset: number): EbmlVintBig {
  ...
  if (offset + width > bytes.length) {
    throw new MkvVintError(offset, `size VINT width ${width} exceeds buffer length`);
  }
...
export function writeVintId(id: number): Uint8Array {
  ...
  throw new MkvVintError(0, `Cannot encode ID 0x${id.toString(16)} as VINT`);
}
...
export function writeVintSize(size: bigint, width?: number): Uint8Array {
  ...
  if (actualWidth > MAX_VINT_WIDTH) {
    throw new MkvVintError(0, `Size ${size} requires VINT width > 8 bytes`);
  }
