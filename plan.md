...
- **License:** MIT
- **Status:** **Phase 1: 7/8 · Phase 2: 7/8 · Phase 3: first-pass containers ✅; `container-mp4` second-pass ✅ (video + fragmented + multi-track + edit-lists + iTunes-metadata + `sidx`/`mfra` typed-parse; only DRM still out); `webm` second-pass (AV1 ✅; multi-track/subtitles/chapters still out) + `mkv` second-pass + `ts` second-pass (M2TS ✅; multi-program/HEVC/SI still out) + FFmpeg-interop tests still pending · Phase 4: 5/5 ✅ COMPLETE · Phase 4.5: 11/N (image-legacy TIFF + TGA + XBM + PCX + XPM + ICNS, data-text JSONL + TOML + FWF + XML + YAML) · Phase 5: v0.1.0 launched 2026-04-23 ✅ (21 packages live on npm) · Phase 6 (Wave B — modern image codecs): ✅ COMPLETE on `main` — AVIF + JXL + HEIC + MozJPEG + OxiPNG + image→PDF + `convertBatch` + CI/CD (npm tag/publish pending)** · CI green · 4,477 tests passing · 27 publishable packages (+ `test-utils`) + 2 apps · last revised 2026-06-29
...
| `container-webm` / `container-mkv` second-pass (multi-track, subtitles, chapters, encryption, AV1 [webm ✅], lacing 10+11) | Phase 3.5 — webm AV1 (V_AV01 + av1C CodecPrivate) done; multi-track/subtitles/chapters/mkv-AV1 still out |
