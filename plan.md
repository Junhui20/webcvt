...
- **License:** MIT
- **Status:** **Phase 1: 7/8 (1 deferred to Phase 5) · Phase 2: 7/8 (5/5 audio containers done — Phase-2 demo deferred to Phase 5) · Phase 3: 3/6 (mp4 + webm + mkv first-passes done; ts + mp4 second-pass + ebml-extraction remain)** · CI green · 1,635 tests passing · last revised 2026-04-19
...
Repo live at https://github.com/Junhui20/webcvt. Phase 1 + Phase 2 library code complete; demo deferred to Phase 5. **Phase 3 in progress: mp4 + webm + mkv first-passes all shipped 2026-04-19.** 14 packages, 1,635 tests, lint+typecheck+build all green in CI.
...
**`container-ts` first-pass** (MPEG-TS + PAT/PMT, transport stream demux/serialize). 188-byte fixed packet structure with sync byte + adaptation/payload sections. Needs: design note (planner agent), TS fixture (likely H.264 + AAC over MPEG-TS), 5-stage pipeline. Smallest of the Phase-3 containers.

### Phase 3 remaining

| Container / task | First-pass LOC | Pipeline status |
|---|---|---|
| `container-ts` (MPEG-TS + PAT/PMT) | ~1,000 | 🔜 next — design note + fixture pending |
| Extract `@webcvt/ebml` from webm + mkv duplicated primitives | ~370 | Phase 3 wrap-up — both consumers exist now |
