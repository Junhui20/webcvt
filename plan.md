...
- [x] `@catlabtech/webcvt-container-mp4` **second-pass** ✅ — edit lists (elst), fragmented MP4 (moof/trun/mvex/tfhd), video tracks (avc1/avc3/hev1/hvc1/vp09/av01 via avcC/hvcC/vpcC/av1C + visual sample entries), multi-track, iTunes metadata (udta/meta/ilst), and **D.3 `sidx` (Segment Index) + `mfra`/`tfra`/`mfro` (Movie Fragment Random Access) typed parse** (surfaced on `Mp4File.sidx[]` / `Mp4File.mfra`; read-only index, preserved byte-identical via the fragmented tail). Package grew 179 → **503 tests** (sidx 100% / mfra ~98% cov). **DRM/CENC signalling** now parsed read-only: `pssh` (Protection System Specific Header) + `sinf`/`frma`/`schm`/`tenc` track encryption → surfaced on `Mp4File.protection` (`encv`/`enca` are unwrapped + redispatched so the track stays usable). No decryption.
...
- [x] `@catlabtech/webcvt-font` — sfnt (TTF/OTF) ↔ **WOFF 1.0** container repackaging (clean-room from OpenType/ISO 14496-22 + WOFF 1.0; zero deps). `parseSfnt`/`serializeSfnt` (recomputes table checksums + `head.checkSumAdjustment`), `parseWoff`/`serializeWoff` (per-table deflate via `Compression/DecompressionStream`), `readFontMeta` (name/head/maxp basics). `FontBackend` (ttf/otf↔woff, no auto-register; output ext follows the sfnt flavor — `OTTO`→otf). Core: `ttf`/`otf`/`woff` formats + `detect.ts` magics (`wOFF`/`OTTO`/`0x00010000`/`true`; no ICO collision). 68 tests, 98.13% stmts / 86.23% branch. Caps: 64 MiB input, ≤4096 tables, per-table + cumulative decompression-bomb caps; bounds-checked offsets. **WOFF2 deferred** (`wOF2` → typed error — needs Brotli, absent from `DecompressionStream`, + glyf transform); ttf↔otf outline/CFF conversion out of scope.
- [x] `@catlabtech/webcvt-doc-ebook-epub` — self-written read-only EPUB reader (EPUB 3.3 OCF + OPF) that **composes** the existing hardened packages: `archive-zip` (`parseZip`, zip-slip + bomb caps) for the OCF ZIP and `data-text` (`parseXml`, DOCTYPE/ENTITY/XXE rejection) for container.xml + OPF. `parseEpub` (async — entry reads are async) → metadata + ordered spine + manifest; `EpubBackend` (epub→txt/html/json, no auto-register). Core: `epub` (`application/epub+zip`) format + `detect.ts` OCF disambiguation (ZIP whose first entry is an uncompressed `mimetype`=`application/epub+zip`). 68 tests, 98.01% stmts / 93.91% branch. Caps: 256 MiB input, ≤10k manifest / ≤5k spine items, 64 MiB output, depth-64 XML walk, `../` traversal rejected. Out of scope: EPUB authoring, recursive nav/NCX TOC, font/CSS rendering.
...
- [x] `@catlabtech/webcvt-comic` — comic archives. **CBZ** (ZIP of images) fully supported: `parseComic` composes `archive-zip` (`parseZip`) → natural-sorted image pages; `ComicBackend` converts `cbz` → **PDF** by composing `doc-pdf`'s `imagesToPdf`. CBR (RAR) and CB7 (7z) are detected by magic and rejected with typed `ComicRarNotSupportedError`/`Comic7zNotSupportedError` (need a RAR/7z wasm decoder — deferred, like WOFF2). Core: `cbz`/`cbr`/`cb7` formats + CBR/CB7 magic detection. 38 tests, 100% lines / 99.15% branch. Caps: 512 MiB input, ≤5000 pages. (Replaces the separate `archive-7z`/`archive-rar` plan entries — `cbr`/`cb7` decode is the deferred wasm part.)
...
| ~~`container-mp4` second-pass (video, fragmented, edit lists, multi-track, iTunes metadata, `sidx`/`mfra`, DRM/CENC signalling)~~ | ✅ done — 503 tests; DRM/CENC `pssh`/`tenc` parsed read-only (no decryption) |
| ~~`container-webm` / `container-mkv` second-pass (AV1, multi-track, subtitles, chapters, tags)~~ | ✅ done — AV1 + multi-track both; mkv adds subtitle tracks (S_TEXT/UTF8/ASS/SSA) + Chapters + Tags. webm 143 / mkv 263 tests. Still out: encryption, lacing 10+11 |
| `container-ts` second-pass (~~M2TS (192-byte BDAV/AVCHD)~~ ✅; multi-program, HEVC, AC-3, SI tables still out) | Phase 3.5 — M2TS done (strip TP_extra_header → 188-byte TS; core detects .m2ts/.mts magic); 156 tests |
