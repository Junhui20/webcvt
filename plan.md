...
| ~~`container-mp4` second-pass (video, fragmented, edit lists, multi-track, iTunes metadata, `sidx`/`mfra`)~~ | ✅ done — 465 tests; **only DRM (cenc/pssh) still out** |
| `container-webm` / `container-mkv` second-pass (multi-track, subtitles, chapters, encryption, ~~AV1~~ ✅ [webm + mkv], lacing 10+11) | Phase 3.5 — AV1 (V_AV01) done in both webm + mkv (mkv also derives the `av01.…` codec string); multi-track/subtitles/chapters still out |
